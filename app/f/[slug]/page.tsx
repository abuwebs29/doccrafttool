...
export default function PublicFormPage() {
  ...
  if (submitted) return <Success message={form.successMessage} redirectUrl={form.redirectUrl} />;
  ...
  function validateQuestions(questions: Question[]) {
    for (const question of questions) {
      ...
      if (question.required && empty) { setError(`Please answer “${question.title}” before continuing.`); return false; }
      if (empty) continue;
      const text = Array.isArray(value) ? value.join(", ") : String(value);
      if (question.type === "url") { try { new URL(text); } catch { setError(`Please enter a valid URL for “${question.title}”.`); return false; } }
      if (question.type === "number") { const number = Number(text); if (!Number.isFinite(number) || (question.minValue != null && number < question.minValue) || (question.maxValue != null && number > question.maxValue)) { setError(`Please enter a valid number for “${question.title}”.`); return false; } }
      if (question.type === "date" && ((question.minDate && text < question.minDate) || (question.maxDate && text > question.maxDate))) { setError(`Please enter a permitted date for “${question.title}”.`); return false; }
    }
    setError("");
    return true;
  }
