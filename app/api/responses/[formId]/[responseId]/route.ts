...
export async function GET(_request: Request, context: { params: Promise<{ formId: string; responseId: string }> }) {
  ...
  const { data, error } = await getSupabaseAdmin().from("form_responses").select("id,form_id,submitted_at,answers,total_score,max_score,reference_number").eq("form_id", formId).eq("id", responseId).maybeSingle();
