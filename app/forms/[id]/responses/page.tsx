...
import { ArrowLeft, BarChart3, ChevronLeft, ChevronRight, Download, Eye, Search, Trash2 } from "lucide-react";
...
export default function ResponsesPage() {
  ...
  const visible = useMemo(() => {
    ...
    const filtered = responses.filter((response) => {
      ...
      const matchesText = !q || response.referenceNumber?.toLowerCase().includes(q) || JSON.stringify(response.answers).toLowerCase().includes(q);
  ...
  function exportExcel() {
    const rows = exportItems.map((response) => {
      const row: Record<string, string | number | Date> = {
        "Reference Number": response.referenceNumber || response.id,
    ...
    const headers = rows.length ? Object.keys(rows[0]) : ["Reference Number", "Submitted At", "Total Score", "Maximum Score"];
