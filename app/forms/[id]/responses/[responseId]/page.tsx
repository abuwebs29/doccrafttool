...
import { ArrowLeft, CalendarDays, Hash, Printer, Trophy } from "lucide-react";
...
export default function ResponseDetailPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/responses/${encodeURIComponent(id)}/${encodeURIComponent(responseId)}`, { cache: "no-store" }).then(async (result) => {
      ...
      return item ? { id:item.id, formId:item.form_id, submittedAt:item.submitted_at, answers:item.answers, totalScore:Number(item.total_score), maxScore:Number(item.max_score), referenceNumber:item.reference_number } : null;
...
function Info({ icon: Icon, label, value }: { icon: typeof Hash; label: string; value: string }) { return <div className="card p-5"><div className="flex items-center gap-3 text-violet-700"><Icon size={19}/><p className="text-sm font-semibold">{label}</p></div><p className="mt-3 break-words font-bold">{value}</p></div>; }
