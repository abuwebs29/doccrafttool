...
function FormCard(props: FormItemProps) {
  ...
  return <article className="group relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-lg hover:shadow-slate-200/70">
    ...
    <div className="mt-5 flex gap-2"><Link href={`/forms/${form.id}/edit`} className="btn-primary flex-1 py-2 text-sm"><Pencil size={15} className="mr-2" />Edit</Link><button onClick={() => props.copyLink(form)} className="btn-secondary flex-1 py-2 text-sm">{props.copied ? <Check size={15} className="mr-2" /> : <Copy size={15} className="mr-2" />}{props.copied ? "Copied" : "Copy link"}</button></div>
  </article>;
}
