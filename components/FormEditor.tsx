...
function makeQuestion(sectionId: string, type: QuestionType = "short_text"): Question {
  return { id: crypto.randomUUID(), sectionId, title: "Untitled question", type, required: false, scoreEnabled: false, points: 0, correctAnswers: [], description: "", placeholder: "", minLength: null, maxLength: null, minValue: null, maxValue: null, minDate: null, maxDate: null, customError: "", scaleMin: 1, scaleMax: 5, scaleMinLabel: "", scaleMaxLabel: "", ...(OPTION_TYPES.includes(type) ? { options: ["Option 1", "Option 2"] } : {}) };
}
  ...
  async function persist(publish = false, navigate = false) {
    if (publish) {
      ...
      for (const question of formRef.current.questions) {
        ...
        if (question.minLength != null && question.maxLength != null && question.minLength > question.maxLength) errors.push(`Fix the character limits for “${question.title}”.`);
      }
...
function AdvancedQuestionSettings({ question, onUpdate }: { question: Question; onUpdate: (patch: Partial<Question>) => void }) {
  const textType = ["short_text", "long_text", "email", "url"].includes(question.type);
  const placeholderAllowed = !["multiple_choice", "checkboxes", "dropdown", "rating", "linear_scale", "acknowledgment"].includes(question.type);
  return <div className="ml-9 mt-3 grid gap-3 rounded-xl border border-slate-200 bg-white p-3 sm:grid-cols-2">
    <div className="sm:col-span-2"><label className="label">Help text</label><input className="field" placeholder="Optional explanation shown below the question" value={question.description ?? ""} onChange={(e) => onUpdate({ description: e.target.value })}/></div>
    {placeholderAllowed && <div className="sm:col-span-2"><label className="label">Placeholder</label><input className="field" placeholder="Example answer" value={question.placeholder ?? ""} onChange={(e) => onUpdate({ placeholder: e.target.value })}/></div>}
    {textType && <><div><label className="label">Minimum characters</label><input className="field" type="number" min="0" value={question.minLength ?? ""} onChange={(e) => onUpdate({ minLength: e.target.value ? Number(e.target.value) : null })}/></div><div><label className="label">Maximum characters</label><input className="field" type="number" min="1" value={question.maxLength ?? ""} onChange={(e) => onUpdate({ maxLength: e.target.value ? Number(e.target.value) : null })}/></div></>}
    {question.type === "number" && <><div><label className="label">Minimum value</label><input className="field" type="number" value={question.minValue ?? ""} onChange={(e) => onUpdate({ minValue: e.target.value ? Number(e.target.value) : null })}/></div><div><label className="label">Maximum value</label><input className="field" type="number" value={question.maxValue ?? ""} onChange={(e) => onUpdate({ maxValue: e.target.value ? Number(e.target.value) : null })}/></div></>}
    {question.type === "date" && <><div><label className="label">Earliest date</label><input className="field" type="date" value={question.minDate ?? ""} onChange={(e) => onUpdate({ minDate: e.target.value || null })}/></div><div><label className="label">Latest date</label><input className="field" type="date" value={question.maxDate ?? ""} onChange={(e) => onUpdate({ maxDate: e.target.value || null })}/></div></>}
    {["rating", "linear_scale"].includes(question.type) && <><div><label className="label">Scale minimum</label><input className="field" type="number" min="0" max="10" value={question.scaleMin ?? 1} onChange={(e) => onUpdate({ scaleMin: Number(e.target.value) })}/></div><div><label className="label">Scale maximum</label><input className="field" type="number" min="2" max="10" value={question.scaleMax ?? 5} onChange={(e) => onUpdate({ scaleMax: Number(e.target.value) })}/></div></>}
    <div className="sm:col-span-2"><label className="label">Custom validation message</label><input className="field" placeholder="Please enter a valid answer." value={question.customError ?? ""} onChange={(e) => onUpdate({ customError: e.target.value })}/></div>
  </div>;
}
...
function SchedulePanel({ form, onChange }: { form: FormRecord; onChange: (form: FormRecord) => void }) {
  ...
  return <div className="mt-5 space-y-4">
    ...
    <div className="card p-6"><h2 className="text-xl font-bold">Response controls</h2><div className="mt-5 grid gap-5 md:grid-cols-2"><div><label className="label">Maximum responses</label><input className="field" type="number" min="1" placeholder="Unlimited" value={form.responseLimit ?? ""} onChange={(e) => onChange({ ...form, responseLimit: e.target.value ? Math.max(1, Number(e.target.value)) : null })}/><p className="mt-2 text-xs text-slate-500">Leave blank to accept unlimited responses.</p></div><div><label className="label">Participant name column</label><select className="field" value={form.participantFieldQuestionId ?? ""} onChange={(e) => onChange({ ...form, participantFieldQuestionId: e.target.value || null })}><option value="">Auto-detect name or email</option>{identityQuestions.map((q) => <option key={q.id} value={q.id}>{q.title}</option>)}</select></div><div><label className="label">Reference prefix</label><input className="field" maxLength={12} value={form.referencePrefix ?? "FR"} onChange={(e) => onChange({ ...form, referencePrefix: e.target.value.toUpperCase() })}/><p className="mt-2 text-xs text-slate-500">Example: FR-2026-00001</p></div></div><div className="mt-5 space-y-3"><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={Boolean(form.oneResponsePerEmail)} onChange={(e) => onChange({ ...form, oneResponsePerEmail: e.target.checked })}/>Allow only one response per email</label><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={Boolean(form.oneResponsePerBrowser)} onChange={(e) => onChange({ ...form, oneResponsePerBrowser: e.target.checked })}/>Allow only one response per browser</label></div></div>
    <div className="card p-6"><h2 className="text-xl font-bold">Link security & access</h2><div className="mt-5 grid gap-5 md:grid-cols-2"><div><label className="label">Link expiry</label><input className="field" type="datetime-local" value={localInputValue(form.linkExpiresAt ?? null)} onChange={(e) => onChange({ ...form, linkExpiresAt: e.target.value ? new Date(e.target.value).toISOString() : null })}/><p className="mt-2 text-xs text-slate-500">Optional. The link stops working after this time.</p></div><div><label className="label">Access codes</label><textarea className="field min-h-28" placeholder={'CODE001\nCODE002'} value={(form.accessCodes ?? []).join("\n")} onChange={(e) => onChange({ ...form, accessCodes: e.target.value.split(/\n|,/).map((v) => v.trim()).filter(Boolean) })}/><p className="mt-2 text-xs text-slate-500">One code per line. Codes are case-insensitive.</p></div></div><div className="mt-5 space-y-3"><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={Boolean(form.requireAccessCode)} onChange={(e) => onChange({ ...form, requireAccessCode: e.target.checked })}/>Require an access code</label><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={Boolean(form.oneResponsePerAccessCode)} onChange={(e) => onChange({ ...form, oneResponsePerAccessCode: e.target.checked })}/>Allow only one response per access code</label><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={form.spamProtectionEnabled !== false} onChange={(e) => onChange({ ...form, spamProtectionEnabled: e.target.checked })}/>Enable spam and rapid-submission protection</label></div></div>
