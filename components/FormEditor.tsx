...
function makeQuestion(sectionId: string, type: QuestionType = "short_text"): Question {
  return { id: crypto.randomUUID(), sectionId, title: "Untitled question", type, required: false, scoreEnabled: false, points: 0, correctAnswers: [], includeInCount: true, description: "", minValue: null, maxValue: null, minDate: null, maxDate: null, scaleMin: 1, scaleMax: 5, scaleMinLabel: "", scaleMaxLabel: "", ...(OPTION_TYPES.includes(type) ? { options: ["Option 1", "Option 2"] } : {}), ...(type === MATRIX_TYPE ? { matrixRows: ["Statement 1", "Statement 2", "Statement 3"], matrixColumns: ["Excellent", "Very Good", "Good", "Fair", "Weak"] } : {}) };
}
...
function SchedulePanel({ form, onChange }: { form: FormRecord; onChange: (form: FormRecord) => void }) {
  ...
  return <div className="mt-5 space-y-4">
    ...
    <div className="card p-6"><h2 className="text-xl font-bold">Question scoring</h2><label className="mt-4 flex items-center gap-3 font-semibold"><input type="checkbox" checked={Boolean(form.scoringEnabled)} onChange={(e) => onChange({ ...form, scoringEnabled: e.target.checked })}/>Enable optional numeric scoring</label><p className="mt-3 text-sm text-slate-500">Only questions with scoring enabled contribute to the total. There is no pass/fail rule.</p><div className="mt-5 space-y-3 border-t border-slate-200 pt-5"><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={form.showScoreAfterSubmission !== false} onChange={(e) => onChange({ ...form, showScoreAfterSubmission: e.target.checked })}/>Show score after submission</label><label className="flex items-center gap-3 font-semibold"><input type="checkbox" checked={form.allowPdfDownload !== false} onChange={(e) => onChange({ ...form, allowPdfDownload: e.target.checked })}/>Allow participant to download PDF</label></div></div>
